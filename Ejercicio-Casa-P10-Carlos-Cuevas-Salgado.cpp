...
/*------------- Alumno: Carlos Cuevas Salgado
	...
	Objetivo: Mover un modelo en un escenario
	Instrucciones : Para moverse atraves de la pnatalla se utilizaran las siguiente teclas :
		  ...
		  Con la tecla P el globo avanza
		  Con la tecla O el globo se detiene

		  **Por default el globo avanza.
...
//Animacion
bool play = true;
float movimientoX = 1.0f;
float movimientoZ = 2.0f;
float rotacion = 0;
...
void animate(void)
{
	if (play) {
		...
		if (movGZ < 1000.0f && movGX == -85.0) {
			movGZ += movimientoZ;
		}
		else if (movGZ > -900.0f && movGX == 50.0f) {
			movGZ -= movimientoZ;
		}
		else if (movGZ == 1000.0f && movGX <= 50.0f) {
			movGX += movimientoX;
		}
		else if (movGZ == -900.0f && movGX >= -85.0f) {
			movGX -= movimientoX;
		}

		

		//rotacion += movimiento * 10;
	}
	
	
}

void display(Shader shader, Model modelo, Model globo)
{
	shader.use();
	...
	model = glm::translate(model, glm::vec3(movGX, 40.0f, movGZ));
...
int main()
{
	...
	// Load models
	Model ourModel =((char *)"Models/carril.obj");
	Model GloboModel = ((char *)"Models/Globo/11809_Hot_air_balloon_l2.obj");
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		display(modelShader, ourModel, GloboModel);
